...
onMounted(() => {
  if (!ballot.value.length) store.dispatch('ballot/fetchBallots', 2);
});
      ...
      <div v-if="currentBallot">
        ...
        >
          <div v-for="ballotState in ballotStates" :key="ballotState.state">
            <time
              datetime="2021-08"
              class="flex items-center text-sm font-serif font-bold leading-6 text-secondary-800"
            >
              <svg
                viewBox="0 0 4 4"
            ...
            <div class="mt-4 text-base leading-8 tracking-tight text-gray-900">
              <div
